...
import { searchViatorProducts } from '@/lib/services/viatorSearch'
...
export async function GET(request: Request) {
  try {
    ...
    if (action === 'search') {
      ...
      try {
        const body = await searchViatorProducts({ query, currency, page, count })
        console.log(
          `[Viator search] q="${query}" total=${body.totalCount} returned=${body.products.length} src=${body.source}`
        )
        return NextResponse.json(body)
      } catch (err) {
        console.error(
          `[Viator search] FAIL q="${query}"`,
        ...
        return NextResponse.json(
          {
            ...
            ...(dev ? { detail: err instanceof Error ? err.message : 'unknown' } : {}),
          },
          { status: 502 }
        )
      }
